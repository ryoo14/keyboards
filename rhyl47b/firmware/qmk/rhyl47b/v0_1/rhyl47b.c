...
layer_state_t default_layer_state_set_kb(layer_state_t state) {
    if (set_scrolling) {
        set_scrolling = false;
        pointing_device_set_cpi(RHYL47B_POINTING_CPI);
    }
...
layer_state_t layer_state_set_kb(layer_state_t state) {
    ...
    if (highest_layer == scroll_layer) {
        set_scrolling = true;
        pointing_device_set_cpi(RHYL47B_SCROLL_CPI);
    } else {
        if (set_scrolling) {
            set_scrolling = false;
            pointing_device_set_cpi(RHYL47B_POINTING_CPI);
...
report_mouse_t pointing_device_task_kb(report_mouse_t mouse_report) {
    if (set_scrolling) {
        ...
        mouse_report.h     = h / RHYL47B_SCROLL_DIVISOR;
        scroll_h_remaining = h % RHYL47B_SCROLL_DIVISOR;
        mouse_report.v     = v / RHYL47B_SCROLL_DIVISOR;
        scroll_v_remaining = v % RHYL47B_SCROLL_DIVISOR;
        mouse_report.x     = 0;
        mouse_report.y     = 0;
    } else {
